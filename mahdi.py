...
# --- دالة تحويل الصور إلى Base64 لتعمل داخل السلايدر ---
...
.slider-container::-webkit-scrollbar-track { background: #F0FDF4; border-radius: 10px; }
.slider-container::-webkit-scrollbar-thumb { background-color: #10B981; border-radius: 10px; border: 2px solid #F0FDF4; }
.slider-container::-webkit-scrollbar-thumb:hover { background-color: #059669; }

/* تصميم بطاقة المنتج داخل السلايدر */
.slider-card {
    flex: 0 0 260px;
...
# ==========================================
# 4. قسم تعريف المنتجات (السلايدر التفاعلي)
# ==========================================
...
slider_html = '<div class="slider-container" dir="rtl">'
...
for prod in placeholders:
    ...
    if img_b64:
        ...
        img_tag = f'<img src="data:{mime_type};base64,{img_b64}" class="slider-img" alt="{prod["name"]}">'
    else:
        img_tag = '<div style="font-size: 5rem; color: #10B981; height: 220px; display: flex; align-items: center; justify-content: center;">🧴</div>'
    
    slider_html += f"""
    <div class="slider-card">
        <div>
            {img_tag}
            <h4 style="color: #10B981; margin: 10px 0 5px 0; font-family: 'Cairo', sans-serif;">{prod['name']}</h4>
        </div>
        <p style="font-size: 0.9rem; color: #444; margin: 0;">{prod['desc']}</p>
    </div>
    """
